...
const {id,name,title,release_date, first_air_date, vote_count,backdrop_path, poster_path, overview}= singleCardEle;
...
console.log("khjhsjk",singleCardEle)
  return (
    <div className='group cursor-pointer sm:hover:shadow-slate-400 sm:shadow-md rounded-lg sm:border-slate-400 sm:m-2 transition-shadow duration-200 p-2'>
      ...
      <p className='flex gap-3 items-center'>{release_date || first_air_date} {" "} <FiThumbsUp className='h-5 m-1'/> {" "} {vote_count}</p>
      </div>
      </Link>
    </div>
  )
